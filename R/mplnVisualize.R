...
#'  mixtures of multivariate Poisson-log normal (MPLN) model.
#'
...
#'    mpln().
#' @param name Name for the plot being generated.
#' @param plots A character string indicating which plots to be produced.
#'    Options are 'heatmaps', 'lines', and 'all'. Default is 'all'.
#' @param Xpanels A positive number indicating the number of panels on x axis.
#'    Default NA.
#' @param Ypanels A positive number indicating the number of panels on y axis.
#'    Default NA.
#' @param format Character string indicating the format of the image to
#'    be produced. Default 'pdf'.
#'
#' @return Plotting function provides the possibility for line and heatmap plots.
#'
#' @export
#' @import graphics
...
mplnVisualize <- function(dataset, clusterMembershipVector,
                          name = '', plots = 'all',
                          Xpanels = NA, Ypanels = NA, format='pdf') {

  selectPLTformat <- function(filename, format = 'png') {
  ...
  # Heatmap 1
  selectPLTformat(paste0(pathNow, "/Clustering_Heatmap1_",name), format)
  gplots::heatmap.2(as.matrix(dataset[vec, ]), dendrogram = "column",
    trace = "none", scale = "row", Rowv = FALSE, col = rev(redgreen(75)),
  ...
  selectPLTformat(paste0(pathNow, "/Clustering_Heatmap2_", name), format)
