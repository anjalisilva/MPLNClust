...
mplnVisualize <- function(dataset, clusterMembershipVector, 
                          name = '', plots = 'all', 
                          Xpanels = NA, Ypanels = NA, FMT='pdf') {
  ...
  if ( (! is.na(Xpanels) && ! is.numeric(Xpanels)) || 
      (is.numeric(Xpanels) && Xpanels <= 0) ) {
	    stop("Xpanels argument should be a positive number!")
  }
  if ((! is.na(Ypanels) && ! is.numeric(Ypanels)) ||
      (is.numeric(Ypanels) && (Ypanels <= 0))) {
  ...
  for (i in 1:max(clusterMembershipVector)) {
    ...
    anothervector[[i]] <- rep(i, 
                length(which(DataPlusLabs[, ncol(dataset) + 1] == i)))
  }
  ...
  heatmap.2(as.matrix(dataset[vec, ]), dendrogram = "column", trace = "none", 
            scale = "row", Rowv = FALSE, col = rev(redgreen(75)), 
  ...
  pheatmap(as.matrix(dataset[vec, ]), show_colnames = T, 
           labels_col = colnames(dataset), annotation_row = annotation_row, 
           fontface = "italic", legend = T, scale ="row", 
           border_color = "black", cluster_row = FALSE, cluster_col = FALSE, 
  ...
  if (is.numeric(Xpanels) && is.numeric(Ypanels)) {
	# bath Xpanels and Ypanels specified by user
	 par(mfrow = c(Ypanels, Xpanels))
		...
		# default case: neither Xpanels nor Ypanels 
	  # specified --> tile in sqrt(N)xsqrt(N)
  ...
  for(cluster in unique(clusterMembershipVector)) {
    selectPLTformat(paste0(pathNow,"/Clustering_LinePlots_Cluster",
                           cluster, "_", name), FMT)
    
      # Save how many observations below to each cluster size, given by 'cluster'
      toplot_1 = as.matrix(DataPlusLabs[which(DataPlusLabs[, ncol(dataset) + 1]
                           == cluster),c(1:ncol(dataset))], ncol = ncol(dataset))
      ...
      if (cluster == 7) {
        ...
        matplot(t(toplot1), type = "l", pch = 1, 
                ...
                xlim = c(1,ncol(toplot1)), main = paste("Cluster ",cluster))
      } else {
      matplot(t(toplot1), type="l", pch=1, col=c(rep(cluster,nrow(toplot_1)),7),
              xlab="Samples", ylab="Expression (log counts)", cex=1, 
              lty=c(rep(2,nrow(toplot_1)),1),lwd=c(rep(1,nrow(toplot_1)),3), 
              xaxt="n", xlim=c(1,ncol(toplot1)), main=paste("Cluster ",cluster))}
